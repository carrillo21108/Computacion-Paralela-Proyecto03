...
#include "common/pgm.h" // Para leer la imagen y dibujar la imagen

const int degreeInc = 2; // Incremento de grados
const int degreeBins = 180 / degreeInc; // Numero de bins
const int rBins = 100; // Numero de bins para el radio
const float radInc = degreeInc * M_PI / 180; // Incremento del radio pi * incremento de grados/180
//*****************************************************************
/*
...
int main (int argc, char **argv)
{
  ...
  float* d_Cos; // Puntero a valores precalculados de cosenos
  float* d_Sin; // Puntero a valores precalculados de senos

  cudaMalloc ((void **) &d_Cos, sizeof (float) * degreeBins); // Reservar memoria para los cosenos
  cudaMalloc ((void **) &d_Sin, sizeof (float) * degreeBins); // Reservar memoria para los senos
  ...
  cudaMemcpy(d_Cos, pcCos, sizeof (float) * degreeBins, cudaMemcpyHostToDevice); // Copiar valores precalculados de cosenos a la GPU
  cudaMemcpy(d_Sin, pcSin, sizeof (float) * degreeBins, cudaMemcpyHostToDevice); // Copiar valores precalculados de senos a la GPU
  ...
  h_hough = (int *) malloc (degreeBins * rBins * sizeof (int)); // Reservar memoria para el acumulador en la CPU
  ...
  // GPU calculation
  GPU_HoughTran <<< blockNum, 256 >>> (d_in, w, h, d_hough, rMax, rScale, d_Cos, d_Sin); // Llamar al kernel de la GPU y ejecutarlo para calculo de acumulador
  ...
  // Dibujar la imagen
  drawImage("houghGlobalCPU.jpg", inImg.pixels, w, h, threshold, cpuht, rScale, rMax); // Dibujar imagen con el acumulador de la CPU
